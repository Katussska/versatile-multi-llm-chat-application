...
export class Migration20260423071626 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      `alter table "user" add column "monthly_dollar_limit" double precision not null default 5;`,
    );
    this.addSql(
      `update "user" set "monthly_dollar_limit" = coalesce("monthly_limit", 5);`,
    );
    this.addSql(
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(
      `update "user" set "admin" = ("role" = 'ADMIN'::user_role), "monthly_limit" = "monthly_dollar_limit";`,
    );
    this.addSql(
      `alter table "user" drop column "role", drop column "monthly_dollar_limit";`,
    );
